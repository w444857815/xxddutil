...
import com.dxhy.order.model.ExcelReadContext;
import com.dxhy.order.model.ExcelReadHandle;
import com.dxhy.order.model.excelHeads.OrderImportExcelEnum;
...
public class ExcelReadServiceImpl implements ExcelReadService {
    ...
    private static String  finalRange= "填表人：                                          抽样员：                                      录入员：                                           复核员：                                                 审计员： ";
    ...
    public List<NewOrderExcel> readOrderInfoFromExcelxls(MultipartFile file) throws OrderReceiveException, ExcelReadException, IOException {
        ...
        //从实体  里获取头部信息 ，excel里的头部只能比枚举类里的 属性少或等
        ...
        return handle.readFromExcel(file.getInputStream(), NewOrderExcel.class);
    
    }


    @Override
    public <T> List<T> transExcelToList(MultipartFile file, Class<T> tClass, Map<String, String> headMap) throws ExcelReadException {
        return null;
    }
    ...
    @Override
    public void exportInvoiceDetailExcel(File file, OutputStream out, Map<String, Object> map, List<String> shList) throws FileNotFoundException {
    ...
    @Override
    public void exportInvocie(FileInputStream file, OutputStream out, Map<String, Object> newMap, List<String> shList) throws IOException {
    ...
    public <T> List<T> transExcelToList(MultipartFile file, Class<T> tClass, Map<String, String> headMap) throws ExcelReadException {
        ...
        ExcelReadContext context = new ExcelReadContext(tClass, headMap, true);
        if (StringUtils.isBlank(file.getOriginalFilename())) {
            context.setFilePrefix(".xlsx");
        } else {
            context.setFilePrefix(file.getOriginalFilename().substring(file.getOriginalFilename().lastIndexOf(".")));
        ...
        try {
            return handle.readFromExcel(file.getInputStream(), tClass);
        } catch (IOException e) {
            log.error("{}文件流读取异常:{}", LOGGER_MSG, e);
            throw new ExcelReadException(ConfigureConstant.STRING_9999, "文件读取异常");
        }
    ...
    *//**
    ...
    private Map<String, Object> examinData(NewOrderExcel orderExcel, NewOrderExcel lastOrderExcel,String xhfNsrsbh) {
        ...
        if (StringUtils.isBlank(orderExcel.getDdh())) {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_DDH.getCellName(), ExcelErrorMessageEnum.ORDERINFO_NULL, true));
            ...
            isValid = true;
            resultList.add(
                    buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_DDH.getCellName(), ExcelErrorMessageEnum.ORDERINFO_DDH_LENGTH_ERROR, false));
        } else if (!DDHPATTERN.matcher(orderExcel.getDdh()).matches()) {
            isValid = true;
            resultList.add(
                    buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_DDH.getCellName(), ExcelErrorMessageEnum.ORDERINFO_DDHFORMAT_ERROR, false));
        }
    
        // 开票类型(开票种类代码) 必填
        if (StringUtils.isBlank(orderExcel.getFpzlDm())) {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_KPLX.getCellName(), ExcelErrorMessageEnum.ORDERINFO_KPLX_NULL, false));
        ...
        } else if (OrderInfoEnum.ORDER_INVOICE_TYPE_12.getValue().equals(orderExcel.getFpzlDm())) {
            orderExcel.setFpzlDm(OrderInfoEnum.ORDER_INVOICE_TYPE_12.getKey());
        } else if (OrderInfoEnum.ORDER_INVOICE_TYPE_42.getValue().equals(orderExcel.getFpzlDm())) {
            orderExcel.setFpzlDm(OrderInfoEnum.ORDER_INVOICE_TYPE_42.getKey());
        } else {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_KPLX.getCellName(), ExcelErrorMessageEnum.ORDERINFO_UNKNOW_KPLX, true));
        }
        //购方编码校验
        if (StringUtils.isNotBlank(orderExcel.getGhf_id()) && GbkUtils.getGbkLength(orderExcel.getGhf_id()) > OrderInfoContentEnum.CHECK_ISS7PRI_107286.getMaxLength()) {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), OrderImportExcelEnum.ORDERINFO_GFID.getKey(), ExcelErrorMessageEnum.ORDERINFO_GFBM_ERROR, true));
        }
        //购方邮箱校验
        if (StringUtils.isNotBlank(orderExcel.getGhf_yx()) && GbkUtils.getGbkLength(orderExcel.getGhf_yx()) > OrderInfoContentEnum.CHECK_ISS7PRI_107042.getMaxLength()) {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), OrderImportExcelEnum.ORDERINFO_GFYX.getKey(), ExcelErrorMessageEnum.ORDERINFO_GFYX_ERROR, true));
        }
    
        if ((StringUtils.isNotBlank(orderExcel.getGhf_yx()) && !EMAILPATTERN.matcher(orderExcel.getGhf_yx()).matches())) {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), OrderImportExcelEnum.ORDERINFO_GFYX.getKey(), ExcelErrorMessageEnum.ORDERINFO_GFYX_FORMAT_ERROR, true));
        }
        // 购货方企业类型 01 企业 03 个人
        if (StringUtils.isBlank(orderExcel.getGhf_qylx())) {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFQYLX.getCellName(), ExcelErrorMessageEnum.ORDERINFO_ENTERPRISE_TYPE_NULL,
                    false));
        } else {
            if (orderExcel.getGhf_qylx().equals(ConfigureConstant.STRING_GHF_QYLX_QY)) {
                ...
                if (OrderInfoEnum.ORDER_INVOICE_TYPE_0.getKey().equals(orderExcel.getFpzlDm())) {
                    ...
                    resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFQYLX.getCellName(),
                            ExcelErrorMessageEnum.ORDERINFO_SPECIAL_QYLX_ERROR, false));
                }
            } else if(orderExcel.getGhf_qylx().equals(ConfigureConstant.STRING_GHF_QYLX_QT)){
                orderExcel.setGhf_qylx(OrderInfoEnum.GHF_QYLX_04.getKey());
                if (OrderInfoEnum.ORDER_INVOICE_TYPE_0.getKey().equals(orderExcel.getFpzlDm())) {
                    isValid = true;
                    resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFQYLX.getCellName(),
                            ExcelErrorMessageEnum.ORDERINFO_SPECIAL_QYLX_ERROR, false));
                ...
                isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFQYLX.getCellName(),
                        ExcelErrorMessageEnum.ORDERINFO_ENTERPRISE_TYPE_ERROR, false));
            }
        }
        ...
        if (!StringUtils.isBlank(orderExcel.getGhf_mc()) && GbkUtils.getGbkLength(orderExcel.getGhf_mc()) > ConfigureConstant.INT_100) {
            isValid = true;
            resultList.add(
                    buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFMC.getCellName(), ExcelErrorMessageEnum.ORDERINFO_GHFMC_OVERLENGTH, false));
        }
        ...
        if (StringUtils.isNotBlank(orderExcel.getGhf_nsrsbh())) {
            ...
            if (!matcher.matches()) {
                isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFNSRSBH.getCellName(), ExcelErrorMessageEnum.TAX_ERROR, true));
                // 税号长度校验
                    ...
                    .parseInt(OrderInfoEnum.TAXPAYER_ID_LENGTH_14.getKey())
                    && orderExcel.getGhf_nsrsbh().length() != Integer
                    .parseInt(OrderInfoEnum.TAXPAYER_ID_LENGTH_16.getKey())
                    && orderExcel.getGhf_nsrsbh().length() != Integer
                ...
                isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFNSRSBH.getCellName(), ExcelErrorMessageEnum.TAX_LENGTH_ERROR, false));
            }
        }
        ...
        if (StringUtils.isNotBlank(orderExcel.getGhf_dz()) && (GbkUtils.getGbkLength(orderExcel.getGhf_dz())) > ConfigureConstant.INT_100) {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFDZ.getCellName(), ExcelErrorMessageEnum.ORDERINFO_GHFDZ_LENGTH_ERROR, false));
        }
        // 购货方电话 20
        if (!StringUtils.isBlank(orderExcel.getGhf_dh()) && (GbkUtils.getGbkLength(orderExcel.getGhf_dh())) > ConfigureConstant.INT_20) {
            isValid = true;
            resultList.add(
                    buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFDH.getCellName(), ExcelErrorMessageEnum.ORDERINFO_GHFDH_OVERLENGTH, false));
        }
        if (!StringUtils.isBlank(orderExcel.getGhf_dh()) && !PHONE_PATTERN.matcher(orderExcel.getGhf_dh()).matches()) {
            isValid = true;
            resultList.add(
                    buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFDH.getCellName(), ExcelErrorMessageEnum.ORDERINFO_GHFDH_ERROR, false));
        }
        ...
        if (!StringUtils.isBlank(orderExcel.getGhf_dz()) && !StringUtils.isBlank(orderExcel.getGhf_dh()) && (GbkUtils.getGbkLength(orderExcel.getGhf_dz()) + GbkUtils.getGbkLength(orderExcel.getGhf_dh())) > ConfigureConstant.INT_100) {
            isValid = true;
            resultList.add(
                    buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFDZ.getCellName(), ExcelErrorMessageEnum.ORDERINFO_GHFDZ_OVERLENGTH, false));
        }
        ...
        if (!StringUtils.isBlank(orderExcel.getGhf_zh())
            ...
            isValid = true;
            resultList.add(
                    buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFZH.getCellName(), ExcelErrorMessageEnum.ORDERINFO_GHFZH_OVERLENGTH, false));
        ...
        if (!StringUtils.isBlank(orderExcel.getGhf_yh()) && GbkUtils.getGbkLength(orderExcel.getGhf_yh()) > ConfigureConstant.INT_100) {
            ...
            isValid = true;
            resultList.add(
                    buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFYH.getCellName(), ExcelErrorMessageEnum.ORDERINFO_GHFYH_OVERLENGTH, false));
        }
        // 购货方银行 + 购货方帐号 100位
        if (StringUtils.isNotBlank(orderExcel.getGhf_yh()) && StringUtils.isNotBlank(orderExcel.getGhf_zh()) && (GbkUtils.getGbkLength(orderExcel.getGhf_yh()) + GbkUtils.getGbkLength(orderExcel.getGhf_zh())) > ConfigureConstant.INT_100) {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_GHFDZ.getCellName(), ExcelErrorMessageEnum.ORDERINFO_GHFYH_GHFZH_OVERLENGTH, false));
        }
        ...
        SimpleTaxClassCode taxClassEntity = getTaxClassFromMapCache(orderExcel.getSpbm(),true);
        ...
        if (!StringUtils.isBlank(orderExcel.getXmsl())) {
            ...
            if (!matcher1.matches()) {
                isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_XMSL.getCellName(), ExcelErrorMessageEnum.ORDERINFO_XMSL_ERROR, true));
            }
            ...
            if (xmsl.contains(ConfigureConstant.STRING_POINT)) {
                if (xmsl.substring(xmsl.indexOf(ConfigureConstant.STRING_POINT) + 1).length() > ConfigureConstant.INT_8) {
                    ...
                    resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_XMSL.getCellName(),
                            ExcelErrorMessageEnum.ORDERINFO_XMSL_OVERLENGTHEIGHT, true));
        ...
        } else if (StringUtils.isNotEmpty(orderExcel.getXmje()) && Double.parseDouble(orderExcel.getXmje()) > 0 && getTaxClassFromMapCache(orderExcel.getSpbm(),true) != null) {
            isValid = true;
            resultList.add(
        ...
        if (!StringUtils.isBlank(orderExcel.getXmdj()) && GbkUtils.getGbkLength(orderExcel.getXmdj()) > ConfigureConstant.INT_20) {
            isValid = true;
            resultList
                    .add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_XMDJ.getCellName(), ExcelErrorMessageEnum.ORDERINFO_DJ_OVERLENGTH, false));
        }
        ...
        if (!StringUtils.isBlank(orderExcel.getXmdj())) {
            ...
            if (!matcher2.matches()) {
                isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_XMDJ.getCellName(), ExcelErrorMessageEnum.ORDERINFO_DJ_ERROR, true));
                ...
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_XMDJ.getCellName(),
                        ExcelErrorMessageEnum.ORDERINFO_XMDJ_OVERLENGTHEIGHT, true));
                ...
                try {
                    ...
                    if (Math.abs(ce) > ConfigureConstant.DOUBLE_PENNY) {
                        isValid = true;
                        resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_XMDJ.getCellName(), ExcelErrorMessageEnum.ORDERINFO_XMDJ_ERROR,
                                true));
                    }
        ...
        if (!StringUtils.isBlank(orderExcel.getXmje()) && orderExcel.getXmje().length() > ConfigureConstant.INT_16) {
            isValid = true;
            resultList
                    .add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_XMJE.getCellName(), ExcelErrorMessageEnum.ORDERINFO_JE_OVERLENGTH, false));
        }
        if (StringUtils.isBlank(orderExcel.getXmje())) {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_XMJE.getCellName(), ExcelErrorMessageEnum.ORDERINFO_JE_NULL, true));
        } else {
            ...
            if (!matcher3.matches()) {
                isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_XMJE.getCellName(), ExcelErrorMessageEnum.ORDERINFO_JE_ERROR, true));
                ...
                if (lastOrderExcel == null) {
                    ...
                    isValid = true;
                    resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_XMJE.getCellName(), ExcelErrorMessageEnum.ORDERINFO_ZKH_XMJE_ERROR, true));
                }else if(!orderExcel.getSpbm().equals(lastOrderExcel.getSpbm())){
                	isValid = true;
                    resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_SPBM.getCellName(), ExcelErrorMessageEnum.ORDERINFO_ZKH_SPBM_ERROR, true));
        ...
        if (StringUtils.isBlank(orderExcel.getHsbz())) {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_HSBZ.getCellName(), ExcelErrorMessageEnum.ORDERINFO_EXCEL_HSBZ_NULL, false));
            ...
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_HSBZ.getCellName(), ExcelErrorMessageEnum.ORDERINFO_EXCEL_HSBZ_UNKNOWN, false));
        }
        ...
        if (!StringUtils.isBlank(orderExcel.getSl()) && orderExcel.getSl().length() > ConfigureConstant.INT_8) {
            isValid = true;
            resultList
                    .add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_SL.getCellName(), ExcelErrorMessageEnum.ORDERINFO_SL_OVERLENGTH, false));
        }
                ...
                if (orderExcel.getSl().contains(ConfigureConstant.STRING_PERCENT)) {
                    ...
                    if (!DIGIT_PATTERN.matcher(substring).matches()) {
                        ...
                        resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_SL.getCellName(),
                                ExcelErrorMessageEnum.ORDERINFO_SL_FORMAT_ERROR, false));
                    } else if (Double.parseDouble(substring) / ConfigureConstant.INT_100 > 1) {
                        isValid = true;
                        resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_SL.getCellName(),
                                ExcelErrorMessageEnum.ORDERINFO_SL_ERROR, false));
                    } else {
            ...
            if (StringUtils.isBlank(orderExcel.getSpbm())) {
                ...
                if (queryProductList.size() != 1) {
                    ...
                    resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_SPBM.getCellName(),
                            ExcelErrorMessageEnum.ORDERINFO_SSFLBM_ERROR, false));
                } else {
        ...
        if (!StringUtils.isBlank(orderExcel.getSe())) {
            ...
            if (!matcher3.matches()) {
                isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_SE.getCellName(), ExcelErrorMessageEnum.ORDERINFO_9704, false));
            }
            if (orderExcel.getSe().length() > ConfigureConstant.INT_20) {
                isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_SE.getCellName(), ExcelErrorMessageEnum.ORDERINFO_9703, false));
            }
            ...
            if(StringUtils.isNotBlank(orderExcel.getSl())){
                ...
                try {
                    ...
                    log.error("{}数据格式化double失败,异常为{}", LOGGER_MSG, e);
                }
            }
        }
        ...
        if (!StringUtils.isBlank(orderExcel.getBmbbbh()) && orderExcel.getBmbbbh().length() > ConfigureConstant.INT_10) {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_BMBBBH.getCellName(), ExcelErrorMessageEnum.ORDERINFO_9702, false));
        }
        ...
        if (!StringUtils.isBlank(orderExcel.getSpbm())) {
            ...
            if (!matcher4.matches()) {
                isValid = true;
                resultList.add(
                        buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_SPBM.getCellName(), ExcelErrorMessageEnum.ORDERINFO_SSFLBM_ERROR, true));
            } else if (orderExcel.getSpbm().length() > ConfigureConstant.INT_19) {
    
                isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_SPBM.getCellName(),
                        ExcelErrorMessageEnum.ORDERINFO_SSFLBM_LENGTH_EROR, true));
            } else if (orderExcel.getSpbm().length() < ConfigureConstant.INT_19) {
    
                orderExcel.setSpbm(StringUtil.fillZero(orderExcel.getSpbm(), 19));
            }

            SimpleTaxClassCode qtc = getTaxClassFromMapCache(orderExcel.getSpbm(),false);
            if (qtc == null) {
                // resultList.add(buildReturnMap(orderExcel.getRowIndex(),
                // ExcelErroMessageEnum.SUCCESSCODE, false));
                isValid = true;
        ...
        //农产品标识
        if(StringUtils.isNotBlank(orderExcel.getIsNcp())){
            if(!ConfigureConstant.STRING_FOU.equals(orderExcel.getIsNcp().trim()) && !ConfigureConstant.STRING_SHI.equals(orderExcel.getIsNcp().trim())){
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERINFO_NCP.getCellName(),
                        ExcelErrorMessageEnum.ORDERINFO_NCPBZ, false));
            }

            if(ConfigureConstant.STRING_SHI.equals(orderExcel.getIsNcp().trim()) &&
                    OrderInfoEnum.ORDER_INVOICE_TYPE_0.getKey().equals(orderExcel.getFpzlDm())){
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERINFO_NCP.getCellName(),
                        ExcelErrorMessageEnum.ORDERINFO_NCPBZ_FPZL, false));
            }
        ...
        if(StringUtils.isNotBlank(orderExcel.getLslbs())){
        	if(OrderInfoEnum.LSLBS_0.getValue().equals(orderExcel.getLslbs())){
        		...
        		isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_LSLBS.getCellName(), ExcelErrorMessageEnum.ORDERINFO_9705, false));
            }
        }
        ...
        if (!StringUtils.isBlank(zzstsgl) && GbkUtils.getGbkLength(zzstsgl) > ConfigureConstant.INT_50) {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_ZZSTSGL.getCellName(), ExcelErrorMessageEnum.ORDERINFO_9713, false));
        }
        //优惠政策标识只能为0或者1 必填
        if ((ConfigureConstant.STRING_YHZCBS_F.equals(yhzcbs))) {
            ...
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_YHZCBS.getCellName(), ExcelErrorMessageEnum.ORDERINFO_9707, false));
        }
        ...
        if (OrderInfoEnum.YHZCBS_1.getKey().equals(yhzcbs)) {
            if (StringUtils.isBlank(zzstsgl)) {
                isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_ZZSTSGL.getCellName(), ExcelErrorMessageEnum.ORDERINFO_9708, false));
            }
            ...
            if (!StringUtils.isBlank(orderExcel.getSl()) &&
                ...
                isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_LSLBS.getCellName(), ExcelErrorMessageEnum.ORDERINFO_9706, false));
            }
        ...
        *//**
        ...
        if (!StringUtils.isBlank(orderExcel.getSl())) {
            *//**
            ...
            if ((!StringUtils.isBlank(lslbs)) && (ConfigureConstant.DOUBLE_PENNY_ZERO != new BigDecimal(orderExcel.getSl()).doubleValue())) {
                isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_SL.getCellName(), ExcelErrorMessageEnum.ORDERINFO_9709, false));
            }
            //零税率标识为空,税率不能为0
            if ((StringUtils.isBlank(lslbs)) && (ConfigureConstant.DOUBLE_PENNY_ZERO == new BigDecimal(orderExcel.getSl()).doubleValue())) {
                isValid = true;
        ...
        if (result) {
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_ZZSTSGL.getCellName(), ExcelErrorMessageEnum.ORDERINFO_9710, false));
        } else {
            ...
            if (OrderInfoEnum.LSLBS_3.getKey().equals(lslbs) && (!(OrderInfoEnum.YHZCBS_0.getKey().equals(yhzcbs)))) {
                isValid = true;
                resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_ZZSTSGL.getCellName(), ExcelErrorMessageEnum.ORDERINFO_9712, false));
            }
        }
        ...
        if (!StringUtils.isBlank(orderExcel.getBz()) && GbkUtils.getGbkLength(orderExcel.getBz()) > ConfigureConstant.INT_150) {
            isValid = true;
            resultList.add(
                    buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDER_BZ.getCellName(), ExcelErrorMessageEnum.ORDERINFO_BZ_OVERLENGTHEIGHT, false));
        }
        ...
        if (!StringUtils.isBlank(orderExcel.getYwlx())) {
            if (GbkUtils.getGbkLength(orderExcel.getYwlx()) > OrderInfoContentEnum.CHECK_ISS7PRI_107294.getMaxLength()) {
                isValid = true;
                resultList.add(
                        buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERINFO_YWLX.getCellName(), ExcelErrorMessageEnum.ORDERINFO_YWLX_OVERLENGTH, false));
            }
    
        }else{
            //如果业务类型是空，返回错误
            isValid = true;
            resultList.add(buildReturnMap(orderExcel.getRowIndex(), NewExcelEnum.ORDERINFO_YWLX.getCellName(), ExcelErrorMessageEnum.ORDERINFO_YWLX_NULL, true));
        }
    ...
    private SimpleTaxClassCode getTaxClassFromMapCache(String spbm, boolean b) {

        SimpleTaxClassCode taxClassCodeEntity = MapCacheManager.getTaxClassCodeFromMapCache(spbm);
        if(b){
            if(taxClassCodeEntity != null && ConfigureConstant.STRING_Y.equals(taxClassCodeEntity.getCpy())){
    ...
    public static Map<String, Object> buildReturnMap(String rowIndex, String cellName, ExcelErrorMessageEnum errorEnum,
    ...
    *//**
    ...
    public Map<String, Object> examinByMap(Map<String, List<NewOrderExcel>> readOrderInfoFromExcelxls, String xhfNsrsbh) {
        ...
        for (Entry<String, List<NewOrderExcel>> entry : readOrderInfoFromExcelxls.entrySet()) {
            ...
            if (StringUtils.isNotBlank(value.get(0).getSpbm()) && getTaxClassFromMapCache(value.get(0).getSpbm(),true) != null) {
            ...
            for (NewOrderExcel newOrderExcel : value) {
                ...
                if (i == 0 && OrderInfoEnum.ORDER_INVOICE_TYPE_51.getKey().equals(value.get(0).getFpzlDm()) && value.size() > ConfigureConstant.PC_MAX_ITEM_LENGTH) {
                    isValid = true;
                    resultList.add(
                            buildReturnMap(newOrderExcel.getRowIndex(), "订单明细", ExcelErrorMessageEnum.ORDERINFO_ELE_ITEM_OVERLIMIT, false));
                }
                if (isContainsCpySpbm && value.size() > 8 && i == value.size() - 1) {
                    isValid = true;
                    resultList.add(
                            buildReturnMap(newOrderExcel.getRowIndex(), "订单明细", ExcelErrorMessageEnum.ORDERINFO_9717, false));
                }
                SimpleTaxClassCode taxClassFromMapCache = getTaxClassFromMapCache(newOrderExcel.getSpbm(), true);
                boolean result = (isContainsCpySpbm && taxClassFromMapCache == null) ||
                        (!isContainsCpySpbm && taxClassFromMapCache != null);
                if (result) {
                    //成品油商品 不可以添加非成品油的商品编码
                    isValid = true;
                    resultList.add(
                            buildReturnMap(newOrderExcel.getRowIndex(), NewExcelEnum.ORDERITEM_SPBM.getCellName(), ExcelErrorMessageEnum.ORDERINFO_9715, false));
                }
                ...
                if ((boolean) examinData.get("isValid") || isValid) {
                    ...
                    hjje = hjje.add(BigDecimal.valueOf(Double.parseDouble(newOrderExcel.getXmje())));
                    errorList.add(examinData);
                }
                i++;
            ...
            if(hjje != null && hjje.doubleValue() == 0.00) {
                Map<String, Object> buildReturnMap = buildReturnMap(value.get(0).getRowIndex(), NewExcelEnum.ORDERITEM_XMJE.getCellName(), ExcelErrorMessageEnum.ORDER_JE_9718, false);
    ...
    *//**
     ...
     * @return
     *//*
    *//*private TaxClassCodeEntity getClassCodeEntity(String spbm,String type) {
        apiTaxClassCodeService.qu(spbm);
    }*//*
    ...
    *//**
    ...
    public void exportInvoiceDetailExcel(File file, OutputStream out, Map<String, Object> map, List<String> shList)
		...
		try {
            ...
            int currrentPage = 1;
            int pageSize = 5000;
            map.put(ConfigureConstant.STRING_CURR_PAGE, String.valueOf(currrentPage));
            map.put(ConfigureConstant.STRING_PAGE_SIZE, String.valueOf(pageSize));
            
            while (count > 0) {
                final DateTime startTime7 = DateTime.now();
                PageUtils exportAllInvoiceDetailByPage = apiOrderInvoiceInfoService.exportAllInvoiceDetailByPage(map, shList, true);
                ...
                for (OrderInvoiceDetail info : exportAllInfo) {
                    ...
                    if (CollectionUtils.isNotEmpty(info.getOrderItemList())) {
                        ...
                        for (OrderItemInfo itemInfo : info.getOrderItemList()) {
                            ...
                            tableRow.createCell(10).setCellValue(info.getKprq() == null ? "" : DateUtil.format(info.getKprq(),"yyyy-MM-dd HH:mm:ss"));
            ...
            log.error("{}excel导出异常:{}", LOGGER_MSG, e);
        } finally {
			try {
				sxssfWorkbook.close();
				inputOut.close();
			} catch (IOException e) {
                log.error("{}输出流关闭异常:{}", LOGGER_MSG, e);
            }
		}
	}
	...
	*//**
    ...
    public void exportOrderInfo(Map<String, Object> paramMap, OutputStream outputStream, List<String> shList) {
        ...
        for (Map<String, Object> param : orderList) {
            ...
            tableRow.createCell(1).setCellValue(DateUtil.formatDateTime(DateUtil.parse(param.get("createTime") == null ? "" : String.valueOf(param.get("createTime")))));
            tableRow.createCell(2).setCellValue(CommonUtils.getFpzlDmMc(param.get("fpzlDm") == null ? "" : String.valueOf(param.get("fpzlDm"))));
            ...
            tableRow.createCell(5).setCellValue(param.get("xhfNsrsbh") == null ? "" : String.valueOf(param.get("xhfNsrsbh")));
            tableRow.createCell(6).setCellValue(param.get("kphjje") == null ? "" : String.valueOf(param.get("kphjje")));
            tableRow.createCell(7).setCellValue(param.get("kpse") == null ? "" : String.valueOf(param.get("kpse")));
            tableRow.createCell(8).setCellValue(formatDdzt(param.get("ddzt") == null ? "" : String.valueOf(param.get("ddzt"))));
            tableRow.createCell(9).setCellValue(formatSbyy(String.valueOf(param.get("ddzt")), param.get("sbyy") == null ? "" : String.valueOf(param.get("sbyy"))));
    		delColWidth(tableRow,colWidthMap);
            index++;
        }
        //设置列宽
    ...
    *//**
    ...
    public List<CommonOrderInfo> excelToOrderInfo(Map<String, List<NewOrderExcel>> orderExcelMap, Map<String, String> paramMap) throws UnsupportedEncodingException, OrderReceiveException, OrderSeparationException {
        ...
        for (Entry<String, List<NewOrderExcel>> entry : orderExcelMap.entrySet()) {
            ...
            orderExcel2CommonOrderInfo.getOrderInfo().setUpdateTime(createTime);
    ...
    *//**
     ...
     *//*
    @Override
    public void exportInvocie(FileInputStream file, OutputStream out, Map<String, Object> paramMap, List<String> shList) throws IOException {
        final DateTime startTime = DateTime.now();
        // 创建一个工作簿
        final DateTime endTime = DateTime.now();
        final long time = new Duration(startTime, endTime).getMillis();
        ...
        //SXSSFWorkbook sxssfWorkbook = new SXSSFWorkbook(1000);
        SXSSFWorkbook sxssfWorkbook = new SXSSFWorkbook(new XSSFWorkbook(file),10000);
        try {
            sxssfWorkbook.setCompressTempFiles(true);
            // 创建一个表格
            ...
            int currrentPage = 1;
            int pageSize = 5000;
            paramMap.put(ConfigureConstant.STRING_CURR_PAGE, String.valueOf(currrentPage));
            paramMap.put(ConfigureConstant.STRING_PAGE_SIZE, String.valueOf(pageSize));

            while (count > 0) {
                final DateTime startTime7 = DateTime.now();
                PageUtils exportAllInvoiceDetailByPage = apiOrderInvoiceInfoService.exportAllInvoiceDetailByPage(paramMap, shList,false);
                ...
                for (OrderInvoiceDetail info : exportAllInfo) {
                    ...
                    log.debug("处理的明细行：{}", index);
                    Row tableRow = sheet.createRow(index);
                    tableRow.createCell(0).setCellValue(String.valueOf(index));
                    tableRow.createCell(1).setCellValue(StringUtils.isBlank(info.getFpzlDm()) ? "" : info.getFpzlDm());
                    tableRow.createCell(2).setCellValue(StringUtils.isBlank(info.getFpdm()) ? "" : info.getFpdm());
                    tableRow.createCell(3).setCellValue(StringUtils.isBlank(info.getFphm()) ? "" : info.getFphm());
                    tableRow.createCell(4).setCellValue(StringUtils.isBlank(info.getXhfmc()) ? "" : info.getXhfmc());
                    tableRow.createCell(5).setCellValue(StringUtils.isBlank(info.getXhfNsrsbh()) ? "" : info.getXhfNsrsbh());
                    tableRow.createCell(6).setCellValue(StringUtils.isBlank(info.getGhf_mc()) ? "" : info.getGhf_mc());
                    tableRow.createCell(7).setCellValue(StringUtils.isBlank(info.getGhfNsrsbh()) ? "" : info.getGhfNsrsbh());
                    tableRow.createCell(8).setCellValue(info.getKprq() == null ? "" : DateUtil.format(info.getKprq(),"yyyy-MM-dd"));
                    tableRow.createCell(9).setCellValue(StringUtils.isBlank(info.getKphjje()) ? "" : info.getKphjje());
                    tableRow.createCell(10).setCellValue(StringUtils.isBlank(info.getHjbhsje()) ? "" : info.getHjbhsje());
                    tableRow.createCell(11).setCellValue(StringUtils.isBlank(info.getKpse()) ? "" : info.getKpse());
                    index++;
                }
            }
           *//* index++;
            Row row = sheet.createRow(index);
            row.createCell(0).setCellValue(finalRange);*//*
    
            //单元格合并
            // 起始行,终止行,起始列,终止列
            CellRangeAddress cra = new CellRangeAddress(index, index, 0, 9);
