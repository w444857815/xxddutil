...
//		System.out.println(masthead);
    ...
    public static void GUI(String htmlStr) throws IOException {
        ...
        /*htmlStr = "<html>"
                ...
                +       "<a>a里面</a>"
                +       "<h3>h3里面</h3>"
                +       "<p>Parsed HTML into a doc.<span>p里的spanM<a>span里的a</a></span></p>"
                +       "<div>第一个div<p>p里的内容<p></div>"
                +       "<div>第二个div<p>2p里的内容<p></div>"
                +   "</body>"
                + "</html>";*/
        ...
        List<HtmlTree> list = new LinkedList<HtmlTree>();
        ...
        for (int i = 0; i <childElements.get(0).children().size() ; i++) {
            ...
            String tagName = xPath+"/"+childElements.get(0).children().get(i).tagName();
            child.setTag(tagName);
            /*System.out.println("只显示text:"+xPath+"/"+childElements.get(0).children().get(i).text());
            //只是当前，不取子集里面的文字，去除html格式，只有展示文字
            System.out.println("只显示text:"+xPath+"/"+childElements.get(0).children().get(i).ownText());
            System.out.println("只显示text:"+xPath+"/"+childElements.get(0).children().get(i).wholeText());
            System.out.println("只显示text:"+xPath+"/"+childElements.get(0).children().get(i).outerHtml());*/
            child.setNowHtml(xPath+"/"+childElements.get(0).children().get(i).ownText());
            if(ObjectUtil.isNotEmpty(params.get(tagName))){
                params.put(tagName, params.get(tagName)+1);
            }else{
                params.put(tagName,1);
            }
            insertMap.put(tagName,1);
            list.add(child);
            System.out.println(childElements.get(0).children().get(i).tagName()+"???"+childElements.get(0).children().get(i).html());
        }
        htmlTree.setTagChild(list);

        for (int j = 0; j <list.size() ; j++) {
            if(params.get(list.get(j).getTag())==1){
                //不处理
            }else{
                String tag = list.get(j).getTag();
                list.get(j).setTag(list.get(j).getTag()+"["+insertMap.get(tag)+"]");
                insertMap.put(tag,insertMap.get(tag)+1);
            }
        }
        ...
        String s = JsonUtils.getInstance().toJsonString(htmlTree);
        System.out.println("最终结果是:"+s);
        FileUtils.writeStringToFile(new File("E://xx.txt"),s,"UTF-8");
        System.out.println("结束");
        if(true){
            return;
        }
    ...
    private static void buildHtmlTree(String xPath, Document doc, HtmlTree htmlTree) {
        List<HtmlTree> tagChild = htmlTree.getTagChild();
        for (int i = 0; i < tagChild.size(); i++) {
            System.out.println("tag:"+tagChild.get(i).getTag()+",html:"+tagChild.get(i).getNowHtml());

            //如果孩子还有孩子的话，处理孩子
            if(doc.selectXpath(tagChild.get(i).getTag()).get(0).children().size()>0){
                Elements childElements = doc.selectXpath(tagChild.get(i).getTag()).get(0).children();
                List<HtmlTree> list = new LinkedList<HtmlTree>();
                ...
                for (int j = 0; j <childElements.size() ; j++) {
                    HtmlTree child = new HtmlTree();
                    String tagName = tagChild.get(i).getTag()+"/"+childElements.get(j).tagName();
                    child.setTag(tagName);
                    if(ObjectUtil.isNotEmpty(params.get(tagName))){
                        params.put(tagName, params.get(tagName)+1);
                    }else{
                    ...
                    child.setNowHtml(childElements.get(j).ownText());
                    list.add(child);
                    System.out.println(childElements.get(j).tagName()+"???"+childElements.get(j).html());
                }
                ...
                tagChild.get(i).setTagChild(list);
                HtmlTree dguiTree = tagChild.get(i);
                buildHtmlTree(xPath, doc, dguiTree);
            }
